...
namespace AWS.Logger.Core
{
  ...
  public class AWSLoggerCore : IAWSLoggerCore
  {
    ...
    public AWSLoggerCore(AWSLoggerConfig config, string logType)
    {
      ...
      ((AmazonCloudWatchLogsClient) this._client).ExceptionEvent += ServiceClientExceptionEvent;
    ...
    private AWSCredentials DetermineCredentials(AWSLoggerConfig config)
    {
    ...
    private AWSCredentials LookupCredentialsFromProfileStore(AWSLoggerConfig config)
    {
    ...
    public void StartMonitor()
    {
      ...
      Task.Run(async () => { await Monitor(_cancelStartSource.Token); });
    }
    ...
    private async Task<string> LogEventTransmissionSetup(CancellationToken token)
    {
      ...
      var streamExistResponse = await _client.DescribeLogStreamsAsync(new DescribeLogStreamsRequest
      {
        LogGroupName = _config.LogGroup,
        LogStreamNamePrefix = currentStreamName
      }, token);
      
      if (!IsSuccessStatusCode(streamExistResponse))
      {
    ...
    public virtual string GenerateStreamName(IAWSLoggerConfig config)
    {
    ...
    /// <returns></returns>
    public static IEnumerable<string> BreakupMessage(string message)
    {
    ...
    {
      private TimeSpan TimeIntervalBetweenPushes { get; set; }
      private int MaxBatchSize { get; set; }
      ...
      public bool IsSizeConstraintViolated(string message)
      {
        ...
        int prospectiveLength = TotalMessageSize + unicode.GetMaxByteCount(message.Length);
      ...
      {
        Encoding unicode = Encoding.Unicode;
        TotalMessageSize += unicode.GetMaxByteCount(ev.Message.Length);
    ...
    private void ServiceClientExceptionEvent(object sender, ExceptionEventArgs e)
    {
